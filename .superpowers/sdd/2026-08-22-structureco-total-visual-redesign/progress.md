...
- Task 1C: completed locally — Three.js support now covers all 40 registry IDs with 20 new editable scenes for space frames, supports, loads, sections and connections. The deterministic generator produced 80 transparent 900×600 PNGs (40 Day + 40 Night); exact persistent load colors, matte/no-texture geometry, distinct signatures and five-family image routing are covered by tests. Structural-assets focal suite 33/33 PASS, PNG contract PASS, typecheck/protected/build PASS, lint exit 0 with 13 preexisting warnings, and Chromium contact-sheet QA PASS at `reports/evidence/2026-08-22-three-structural-assets/`. Report: `reports/2026-08-22-0238-three-structural-library.md`. No push.
- Task 1C: fix round 1/5 (4 addressed, 1 open — exact MeshStandard validation still accepted glass-like MeshPhysicalMaterial through inheritance; commits `17f92cc..05a179e`).
- Task 1C: fix round 2/5 (1 addressed, 0 open — glass-like `MeshPhysicalMaterial` is rejected by exact material-class validation; commit `d78d177`).
- Task 1: complete (commits `50eae85..d78d177`, independent review clean after two Task 1C fix rounds). Fresh controller gates: structural-assets 8 files/35 tests PASS before round 2; post-round focused 6/6 and PNG/publication contract 6/6 + 80/80 transparent renders PASS; protected boundary 38/38 intact. No push.
- Task 1C fix round 1/5: closed all five Important review findings. PNG validation now proves decoded transparent pixels; all 40 canonical IDs/order, distinct editable signatures and strict opaque matte `MeshStandardMaterial` contracts are enforced; atlas routing is checked rather than cast; generation validates a safe sibling staging bundle before transactional publication and preserves the previous bundle on failure. Fresh evidence: Node contracts 6/6 PASS, focal scene contracts 8/8 PASS, full structural-assets 35/35 PASS, generated/validated PNGs 80/80, Chromium Day/Night QA PASS, typecheck/protected/build PASS, lint exit 0 with 13 preexisting warnings. Task 2 checkpoint completion lines above were preserved. No push.
- Task 3: complete — Illustration Studio uses one canonical transformed Three.js group/camera for live WebGL, transparent SVG and exact 1×/2×/4× PNG. Preset schema v1 is isolated from project data; 40 factory presets are immutable; personal lifecycle, Home Settings desktop/mobile, Escape/focus and bilingual labels are covered. Final evidence: 18 files/90 tests PASS, typecheck/lint/protected/build PASS, Chromium and WebKit Day/Night desktop/mobile QA PASS, no source push.
- Task 3 fix round 1/5: closed the 1 Critical and 18 Important findings from the first independent review — full mobile preset lifecycle, modal focus/inert, safe persistence, visible errors, stable Night theme, exact shared 3:2 projection, transactional export cleanup, preserved technical accents, stronger contrast/localization and materially stronger Chromium/WebKit oracles (`b7d0541`).
- Task 3 final: closed by user decision after fresh controller evidence: 18 files/104 tests PASS, typecheck/lint/build PASS, protected boundary 38/38, eight regenerated captures and QA summaries with zero failures. Additional redundant review was stopped; source remains unpushed. Deferred polish for later responsive pass: clipped mobile preview-tab affordance, eager Home Studio import and orientation/ResizeObserver refinement.
