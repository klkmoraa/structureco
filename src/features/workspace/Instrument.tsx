...
export const Instrument = ({ coordinateReadoutRef, scaleReadoutRef }: InstrumentProps) => {
  ...
  useEffect(() => {
    ...
    void import('../model-doctor/modelDoctorDiagnostics')
      .then(({ buildModelDoctorReport }) => { if (active) setModelDoctorTotal(buildModelDoctorReport(project).total); })
      .catch(() => undefined);
    return () => { active = false; };
  }, [project]);
  ...
  return <footer className="workspace-instrument" aria-label={t('canvas.viewStatus')} data-instrument="true">
    ...
    <div className="instrument-health">
      ...
      />
      <div className={`instrument-storage${storageHasIssue ? ' is-issue' : ''}`} data-storage-state={storageState} title={storageMessage ?? storageLabel} role="status" aria-label={storageMessage ?? storageLabel}>
