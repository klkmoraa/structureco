...
export interface Space3DViewportOptions {
  ...
  readonly createControls?: (camera: THREE.PerspectiveCamera, canvas: HTMLCanvasElement) => Space3DControlsLike;
}
...
const readPalette = (): Record<ColorRole, THREE.Color> => {
  ...
  const entries = Object.entries(TOKEN_COLORS).map(([role, [token, fallback]]) => {
    ...
    try {
      color = new THREE.Color(raw && raw.length > 0 ? raw : fallback);
    } catch {
      color = new THREE.Color(fallback);
    }
    return [role, color] as const;
  });
  return Object.fromEntries(entries) as Record<ColorRole, THREE.Color>;
};
...
const lineGeometry = (values: readonly number[]): THREE.BufferGeometry => {
  const geometry = new THREE.BufferGeometry();
  geometry.setAttribute('position', new THREE.Float32BufferAttribute(values.length > 0 ? [...values] : [], 3));
  return geometry;
};
...
const makeLabel = (text: string, color: THREE.Color, size: number): THREE.Sprite | null => {
...
export const createSpace3DViewport = (options: Space3DViewportOptions): Space3DViewport => {
  ...
  const groups = new Map<Space3DSceneLayer, THREE.Group>();
  ...
  let memberLines: THREE.LineSegments | null = null;
  ...
  const buildStatic = () => {
    ...
    const divisions = 24;
    const grid = new THREE.GridHelper(size, divisions, palette.gridStrong, palette.grid);
    ...
    const origin = new THREE.Vector3(model.bounds.min[0], model.bounds.min[1], model.bounds.min[2]);
    const axes: [THREE.Vector3, THREE.Color, string][] = [
      [new THREE.Vector3(1, 0, 0), palette.axisX, 'X'],
      [new THREE.Vector3(0, 1, 0), palette.axisY, 'Y'],
      [new THREE.Vector3(0, 0, 1), palette.axisZ, 'Z'],
    ];
    for (const [direction, color, letter] of axes) {
      axesGroup.add(new THREE.ArrowHelper(direction, origin, axisLength, color, axisLength * 0.18, axisLength * 0.1));
  ...
  const buildModel = () => {
    ...
    memberGeometry.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3));
    memberLines = new THREE.LineSegments(memberGeometry, new THREE.LineBasicMaterial({ vertexColors: true }));
    ...
    nodeGeometry.setAttribute('position', new THREE.Float32BufferAttribute(model.nodes.flatMap((node) => [...node.position]), 3));
    nodeGeometry.setAttribute('color', new THREE.Float32BufferAttribute(model.nodes.flatMap((node) => {
      const color = node.selected ? palette.nodeSelected : palette.node;
      return [color.r, color.g, color.b];
    }), 3));
    nodePoints = new THREE.Points(nodeGeometry, new THREE.PointsMaterial({
    ...
    for (const support of model.supports) {
      const geometry = new THREE.BoxGeometry(supportSize * 2, supportSize, supportSize * 2);
    ...
    for (const load of model.loads) {
      const direction = new THREE.Vector3(...load.direction);
      ...
      const tail = new THREE.Vector3(...load.origin).addScaledVector(direction, -length);
      loadsGroup.add(new THREE.ArrowHelper(direction, tail, length, color, length * 0.26, length * 0.14));
      if (load.kind === 'moment') {
        // Doble punta: la convención de la regla de la mano derecha.
        const second = new THREE.Vector3(...load.origin).addScaledVector(direction, -length * 0.72);
        loadsGroup.add(new THREE.ArrowHelper(direction, second, length * 0.72, color, length * 0.26, length * 0.14));
      }
    }

    const axesGroup = groups.get('local-axes')!;
    if (model.localAxes) {
      const origin = new THREE.Vector3(...model.localAxes.origin);
      const length = Math.max(model.localAxes.length, span * 0.06);
      const triad: [readonly number[], THREE.Color, string][] = [
      ...
      for (const [vector, color, letter] of triad) {
        ...
        axesGroup.add(new THREE.ArrowHelper(direction, origin, length, color, length * 0.22, length * 0.12));
...
const defaultControls = (camera: THREE.PerspectiveCamera, canvas: HTMLCanvasElement): Space3DControlsLike => {
