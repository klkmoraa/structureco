...
export const ResultSummary = () => {
  ...
  return <section id="classroom-result-summary" className="result-summary-workspace" aria-label={t('results.summaryWorkspace')} tabIndex={-1}>
    ...
    {scenarios ? <section className="scenario-comparison" aria-live="polite"><div className="scenario-comparison-heading"><div><strong>{t('results.scenarioComparison')}</strong><span>{t('results.scenariosSolved', { count: scenarios.length })}</span></div><small>{t('results.smallMultiplesHint')}</small></div><div className="scenario-cards">{scenarios.map((scenario) => { const item = summarizeAnalysisResults(scenario.result); return <article key={scenario.id}><strong>{scenario.name}</strong>{(['axial', 'shear', 'moment'] as const).map((quantity) => <span key={quantity}><b>{diagramSymbol[quantity]}</b>{item.diagrams[quantity] ? displayDiagram(quantity, item.diagrams[quantity]!.absolute.value) : '—'}</span>)}</article>; })}</div><div className="envelope-summary"><span><b>{t('results.reactionEnvelope')}</b>{t('results.nodesCompared', { count: reactionEnvelope?.nodes.length ?? 0 })}</span>{classroom ? null : <span><b>{t('results.deformationEnvelope', { member: selectedMemberId })}</b>{deformationEnvelope ? `${toDisplay(deformationEnvelope.minimum.value, units, 'length').toExponential(2)} → ${toDisplay(deformationEnvelope.maximum.value, units, 'length').toExponential(2)} ${unitLabel(units, 'length')}` : t('results.unavailable')}</span>}</div></section> : null}
