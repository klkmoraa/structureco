...
const STATIC_COMMANDS: readonly CommandDefinition[] = [
  ...
  },
  {
    ...
    category: 'view',
    icon: Keyboard,
    label: (ctx) => ctx.t('shortcuts.title'),
    hint: (ctx) => ctx.t('shortcuts.description'),
    aliases: () => ['atajos', 'teclado', 'shortcuts', 'ayuda', 'help', 'hotkeys'],
    route: () => 'Ayuda › Atajos de teclado',
    shortcut: '?',
    run: () => emitWorkspaceCommand('open-keyboard-shortcuts'),
  },
  {
    id: 'export:bom',
    category: 'export',
