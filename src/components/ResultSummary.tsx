...
export const ResultSummary = () => {
  ...
  const displayDiagram = (quantity: DiagramQuantity, value: number) => {
    ...
    return `${toDisplay(value, units, unitQuantity).toFixed(3)} ${unitLabel(units, unitQuantity)}`;
  };
  return <div className="result-summary-workspace">
    ...
    <div className="global-extrema-grid">
      {(['axial', 'shear', 'moment'] as const).map((quantity) => {
        ...
        return item ? <button className={quantity} key={quantity} onClick={() => locate(quantity, item.memberId, item.x)}><span>{diagramSymbol[quantity]} máx. absoluto</span><strong>{displayDiagram(quantity, item.value)}</strong><small>{item.memberId} · x {toDisplay(item.x, units, 'length').toFixed(3)} {unitLabel(units, 'length')}</small><LocateFixed size={14} /></button> : null;
      })}
      {!classroom && summary.deformations.v?.absolute ? <button className="deformation" onClick={() => { const item = summary.deformations.v!.absolute; locate('v', item.memberId, item.x); }}><span>Desplazamiento v</span><strong>{toDisplay(summary.deformations.v.absolute.value, units, 'length').toExponential(3)} {unitLabel(units, 'length')}</strong><small>{summary.deformations.v.absolute.memberId} · máximo interior exacto</small><LocateFixed size={14} /></button> : null}
    </div>
