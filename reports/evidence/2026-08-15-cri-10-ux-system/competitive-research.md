...
- **cómo podría aplicarse** — Dos aplicaciones distintas. (1) **Chrome** (ToolRail, TopBar, `More`, controles del Inspector): es donde SC 2.5.8 aplica literalmente y **debe cumplirse** — 24 px CSS mínimo o separación equivalente; esto por sí solo condiciona cuántos botones caben y por tanto es un argumento objetivo, no estético, para reducir el chrome permanente. (2) **Canvas**: aunque los objetos dibujados quedan fuera del alcance estricto de la SC, la *lógica* se traslada: el **hit region** de un nodo o barra debe ser mayor que su representación visual, y cuando las hit regions de dos objetos se solapan, el sistema **no debe elegir en silencio** — debe desambiguar (ficha O-2) o abrir el selector de precisión (X-1). El criterio "los círculos no se intersecan" es **exactamente el test computable** para decidir cuándo activar la desambiguación.
- **qué NO debemos copiar** — No tratar 24 px como objetivo de diseño: es un **mínimo legal**, y para uso táctil real con la mano en movimiento es insuficiente (ver X-3). Y no usar las excepciones de la SC (*inline*, *user agent control*, *essential*) como puerta trasera para dejar el ToolRail apretado: en un ToolRail nada es *inline* ni *essential* en el sentido normativo.
...
5. **En táctil, desacoplar el punto de contacto del punto de selección y enumerar la profundidad en vez de adivinarla** (O-1 + O-2 + X-1 + X-2). Onshape documenta el motivo textualmente — *"using a finger as a cursor often obstructs one's view"* — y WCAG 2.2 SC 2.5.8 da el test computable (círculos de 24 px que no deben intersecarse) para saber **cuándo** hay ambigüedad y activar la desambiguación.
