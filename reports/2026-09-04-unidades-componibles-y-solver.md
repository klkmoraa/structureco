...
Pruebas añadidas: coherencia de las magnitudes derivadas y sistemas propios (`units.test.ts`), reúso de factorización y residuo compensado (`math.test.ts`), densidad y fidelidad de la deformada (`diagram.test.ts`), persistencia y validación de los sistemas propios (`migrate.test.ts`) y composición de un sistema desde la barra superior (`TopBar.test.tsx`).

## Abierto

`src/App.test.tsx > renders the editor and runs analysis for an existing model` es inestable en la corrida completa: falla una de cada tres ejecuciones y también lo hace sobre `main` sin estos cambios. No se tocó aquí.
